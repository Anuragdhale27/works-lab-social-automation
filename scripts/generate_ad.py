...
FONTS = {
    "inter": {
        ...
        "regular": "/usr/share/fonts/opentype/inter/InterDisplay-Regular.otf",
    },
    "lato": {
...
THEMES = {
    "electric": {"bg": (10,15,27), "accent": (255,84,145), "ink": (249,251,255), "muted": (181,191,207), "font": "inter", "layout": "bold_left", "dark": True},
    "cream_red": {"bg": (249,243,232), "accent": (221,61,57), "ink": (28,28,31), "muted": (91,86,80), "font": "serif", "layout": "magazine", "dark": False},
    "neo_mint": {"bg": (226,247,239), "accent": (8,126,99), "ink": (16,39,33), "muted": (72,101,93), "font": "lato", "layout": "sticker", "dark": False},
    "cobalt": {"bg": (231,239,255), "accent": (33,84,220), "ink": (12,22,48), "muted": (71,86,112), "font": "inter", "layout": "browser", "dark": False},
    "sunset_purple": {"bg": (55,30,79), "accent": (255,173,70), "ink": (255,248,239), "muted": (220,205,230), "font": "lato", "layout": "split", "dark": True},
    "paper_ink": {"bg": (245,241,232), "accent": (30,31,34), "ink": (22,22,24), "muted": (92,89,84), "font": "condensed", "layout": "ticket", "dark": False},
    "violet_pop": {"bg": (239,229,255), "accent": (117,65,235), "ink": (30,18,55), "muted": (90,77,118), "font": "inter", "layout": "chat", "dark": False},
    ...
    "mono_red": {"bg": (27,27,28), "accent": (235,50,72), "ink": (250,250,250), "muted": (170,170,174), "font": "lato", "layout": "minimal", "dark": True},
}
...
def background(theme):
    ...
    if layout == "browser":
        ...
        for x in range(0,W,120):
            d.line((x,0,x,H), fill=(*theme["accent"],12), width=1)
    ...
    if layout == "ticket":
        for y in (70,1012):
            d.line((70,y,1010,y), fill=(*theme["accent"],170), width=2)
        for x in range(82,1010,28):
            d.ellipse((x-2,998,x+2,1002), fill=(*theme["accent"],150))

    if layout == "chat":
        for y in range(115,980,92):
            d.rounded_rectangle((760,y,1010,y+42), 21, fill=(*theme["accent"],30))

    if layout == "poster":
        d.polygon([(760,0),(1080,0),(1080,1080),(545,1080)], fill=(255,255,255,48))

    if layout == "highlighter":
...
def rich_hook(d, item, theme, x, y, max_w, max_h, size=68):
    text = item["hook"]
    f, txt = fit(d,text,max_w,max_h,size,40,theme,True)
    lines = txt.split("\n")
    yy = y
...
def body_value(d, item, theme, x, y, max_w, body_h, value_h=112):
    ...
    if parts:
        ...
        available = max_w-28
        for part in parts[:3]:
            pf,pt = fit(d,part,min(available,220),value_h-52,18,14,theme,False)
...
def render(item,template,theme_name):
    ...
    if layout == "bold_left":
        ...
        for cx in (95,116,137):
            d.ellipse((cx,181,cx+10,191),fill=(255,255,255,210))
...
if __name__ == "__main__":
    ...
    metadata = {
        ...
        "cta": item["cta"],
        "theme": theme_name,
        "layout": THEMES[theme_name]["layout"],
